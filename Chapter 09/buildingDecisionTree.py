...
def get_splitpoint(allvalues, predictedvalues):
    ...
    for pctl in range(0, 100):
        ...
        loweroutcomes = [outcome for value, outcome in 
                         zip(allvalues, predictedvalues) 
                         if value <= split_candidate]
        higheroutcomes = [outcome for value, outcome in 
                          zip(allvalues, predictedvalues)
                          if value > split_candidate]
...
def getsplit(depth, data, variables, outcome_variable):
    ...
    generated_tree = [[best_var, float('-inf'), best_split, best_lowermean], 
                      [best_var, float('inf'), best_split, best_highermean]]
    ...
    if depth < maxdepth:
        ...
        if len(splitdata1.index) > 10 and len(splitdata2.index) > 10:
            generated_tree[0][3] = getsplit(depth + 1, splitdata1, variables, 
                                            outcome_variable)
            generated_tree[1][3] = getsplit(depth + 1, splitdata2, variables, 
                                            outcome_variable)
        else:
            depth = maxdepth + 1
            generated_tree[0][3] = best_lowermean
            generated_tree[0][3] = best_highermean
    else:
        generated_tree[0][3] = best_lowermean
